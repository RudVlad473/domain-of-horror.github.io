export default class Cart {
    ...
    addItem({thumbnailUrl, title, unitPrice, amount}) {
        ...
        const newItem = `
                    <div id="${
                        currentDOMitems.length + 1
                    }" class="cart__content__item">
                        ...
                        <div class="cart__content__item__desc">
                            ...
                            <div class="cart__content__item__desc__price">
                                ...
                                <span class="cart__content__item__desc__price__sum">
                                    ${Number(unitPrice * amount).toFixed(2)}
                                </span>
                            </div>
                        </div>
                        <div class="cart__content__item__delete" onclick="cart.removeItem(${
                            currentDOMitems.length + 1
                        })">
    ...
    removeItem(index) {
        ...
        if (index >= currentDOMitems.length || index < 0) {
            throw new Error("Item with such index does not exist")
        }

        currentDOMitems[index].remove()
        this.decrementBadge()
    }
    ...
    incrementBadge() {
        if (+this.DOMbadge.innerHTML >= 9) {
            ...
            this.DOMbadge.innerHTML = ++this.DOMbadge.innerHTML
        }
    }
